...
MyApp.controller('ContestsController', function ($scope, $http, $filter, $window, $rootScope, $location) {
    ...
    $scope.SubmitContests = function (type) {
        if (type === 'Login') {
            Login($scope.emailid, $scope.password, 0);
        }
    ...
    function SubmitContests(profileid) {
        try {
            SubmitAnswer(0, 0, "REAL", profileid, $scope.contest_id);
        }
        catch (ex) {
            //ajaxindicatorstop();
        }
    ...
    function SubmitAnswer(QuestionID, Answer, entry_type, person_id, contest_id) {
        try {
            //ajaxindicatorstart('please wait..');
            var _obj = {
